...
export const AdminPortalModal: React.FC<AdminPortalModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    if (passcode.toLowerCase() === 'customink2026') {
      ...
      <div className="relative w-full max-w-4xl bg-[#121214] border border-rose-500/30 rounded-2xl p-6 sm:p-8 shadow-2xl text-zinc-100 max-h-[90vh] overflow-y-auto">
        ...
        {!isAuthenticated ? (
          <div className="py-8 max-w-md mx-auto text-center">
            ...
            <h3 className="text-2xl font-bold tracking-tight mb-2">CUSTOM INK STUDIO OS Portal</h3>
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              <div>
                <input
                  ...
                  placeholder="Enter manager passkey (customink2026)"
              ...
              {error && (
                <p className="text-red-400 text-xs font-mono">Invalid passkey. Use: customink2026</p>
              )}
              ...
              <div className="flex flex-col sm:flex-row gap-3 pt-2">
                ...
                <button
                  ...
                  className="py-3 px-4 bg-zinc-800 hover:bg-zinc-700 text-rose-400 font-mono text-base font-semibold min-h-[44px] rounded-xl border border-rose-500/30 transition-colors"
                >
                  ⚡ Auto-Fill (customink2026)
                </button>
              </div>
            </form>
          </div>
          ...
          <div>
            <div className="flex flex-wrap items-center justify-between gap-4 border-b border-zinc-800 pb-6 mb-6">
              <div>
                ...
                <h3 className="text-xl font-bold text-white">CUSTOM INK STUDIO OS // BACK-OFFICE</h3>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="p-4 rounded-xl bg-zinc-900/60 border border-zinc-800">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                      ...
                      <div key="artist_roster" className="p-2.5 rounded bg-zinc-950 border border-zinc-800 text-xs font-mono text-zinc-300 flex items-center gap-1.5">
                        <CheckCircle className="w-3.5 h-3.5 text-emerald-400 flex-shrink-0" />
                        <span>artist_roster</span>
                      </div>
                    
                      <div key="deposit_bookings" className="p-2.5 rounded bg-zinc-950 border border-zinc-800 text-xs font-mono text-zinc-300 flex items-center gap-1.5">
                        <CheckCircle className="w-3.5 h-3.5 text-emerald-400 flex-shrink-0" />
                        <span>deposit_bookings</span>
                      </div>
                    
                      <div key="digital_waivers" className="p-2.5 rounded bg-zinc-950 border border-zinc-800 text-xs font-mono text-zinc-300 flex items-center gap-1.5">
                        <CheckCircle className="w-3.5 h-3.5 text-emerald-400 flex-shrink-0" />
                        <span>digital_waivers</span>
                      </div>
                    
                      <div key="flash_drops" className="p-2.5 rounded bg-zinc-950 border border-zinc-800 text-xs font-mono text-zinc-300 flex items-center gap-1.5">
                        <CheckCircle className="w-3.5 h-3.5 text-emerald-400 flex-shrink-0" />
                        <span>flash_drops</span>
                      </div>
            ...
            {activeTab === 'telemetry' && (
              <div className="p-4 rounded-xl bg-zinc-900 border border-zinc-800 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div key="RESIDENT MASTER ARTISTS" className="p-3 rounded-lg bg-zinc-950 border border-zinc-800">
                      <span className="text-xs text-zinc-300 font-mono">RESIDENT MASTER ARTISTS</span>
                      <p className="text-lg font-bold text-rose-400 font-mono mt-0.5">{"7 ARTISTS"}</p>
                    </div>
                  
                    <div key="DEPOSIT RETENTION GATE" className="p-3 rounded-lg bg-zinc-950 border border-zinc-800">
                      <span className="text-xs text-zinc-300 font-mono">DEPOSIT RETENTION GATE</span>
                      <p className="text-lg font-bold text-rose-400 font-mono mt-0.5">{"$150 NON-REFUNDABLE"}</p>
                    </div>
                  
                    <div key="HEALTH DEPT COMPLIANCE" className="p-3 rounded-lg bg-zinc-950 border border-zinc-800">
                      <span className="text-xs text-zinc-300 font-mono">HEALTH DEPT COMPLIANCE</span>
                      <p className="text-lg font-bold text-rose-400 font-mono mt-0.5">{"100% STERILE AUDITED"}</p>
                    </div>
                  
                    <div key="DIGITAL WAIVERS ARCHIVED" className="p-3 rounded-lg bg-zinc-950 border border-zinc-800">
                      <span className="text-xs text-zinc-300 font-mono">DIGITAL WAIVERS ARCHIVED</span>
                      <p className="text-lg font-bold text-rose-400 font-mono mt-0.5">{"8,400+ SIGNED"}</p>
                    </div>
            ...
            {activeTab === 'sql' && (
              <div className="p-4 rounded-xl bg-zinc-950 border border-zinc-800 font-mono text-xs text-zinc-300 overflow-x-auto">
                ...
                <div className="text-rose-400">ALTER TABLE artist_roster ENABLE ROW LEVEL SECURITY;</div>
                <div className="text-zinc-400 mt-1">CREATE POLICY "Allow authenticated read" ON artist_roster FOR SELECT USING (true);</div>
